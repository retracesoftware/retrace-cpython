...
overlay plus a `retrace-python` launcher. The workflow's `python_version` input
defaults to `manifest-all`, which builds every CPython release listed in each
patch manifest's `verified_applies`. Use `manifest-latest` only for cheaper
smoke runs, or pass an exact CPython version for one-off debugging. The
workflow's `package_bump` input selects the Retrace package release bump
(`point`, `minor`, or `major`) to apply to the latest PyPI release. If no PyPI
release exists yet, the bump starts from `0.0.0`. This package version is
